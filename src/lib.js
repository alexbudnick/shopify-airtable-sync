...
export function verifyShopifyWebhook(rawBody, hmacHeader) {
  const digest = crypto
    .createHmac("sha256", CFG.shopify.webhookSecret)
    .update(rawBody, "utf8")
    .digest("base64");
...
export async function getShopifyAccessToken() {
  ...
  const url = `https://${CFG.shopify.storeDomain}/admin/oauth/access_token`;
...
export function normalizeShopifyStatus(status) {
  return safeUpper(status);
}
