...
export async function findAirtableRecordByVariantId(variantId) {
  const formula = `{${CFG.airtable.shopifyVariantIdField}}="${String(variantId).replace(/"/g, '\\"')}"`;
  const payload = await airtableRequest(`?filterByFormula=${encodeURIComponent(formula)}&maxRecords=1`);
  return payload.records?.[0] || null;
}
...
export async function getVariantByInventoryItemId(inventoryItemGid) {
  const query = `
